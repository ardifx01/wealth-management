...
class WalletResource extends Resource
{
    ...
    public static function form(Form $form): Form
    {
        ...
        return $form->schema([
            ...
            Forms\Components\TextInput::make('name')
                ->columnSpanFull()
                ->required(),
            Forms\Components\Textarea::make('descrption')->columnSpanFull(),
            Forms\Components\Select::make('category')
                ->options(
                    array_combine(
                        ...
                        array_map(
                            fn($case) => ucfirst(strtolower($case->value)),
                            WalletType::cases(),
                        ),
                    ),
                )
    ...
    public static function table(Table $table): Table
    {
        ...
        return $table
            ->columns([
                Tables\Columns\TextColumn::make('name')->description(function (
                    Wallet $record,
                ) {
                ...
                Tables\Columns\TextColumn::make('currency')->description(
                    function (Wallet $record) use ($detailCurrencies) {
                        return collect($detailCurrencies)->get(
                            $record->currency,
                        )['name'];
                    },
                ),
                Tables\Columns\TextColumn::make('balance')->formatStateUsing(
                    fn(Wallet $record) => Number::currency(
                        $record->balance,
                        $record->currency,
                    ),
                ),
